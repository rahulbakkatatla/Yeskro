      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-white mx-5 mt-5 rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 flex gap-4">
            ...
            <div className="text-center">
              <div className="text-xl font-black text-teal-600">{listings.filter(l => l.type === 'offering').length}</div>
              <div className="text-xs text-gray-400">Offering</div>
            </div>
            <div className="text-center">
              <div className="text-xl font-black text-amber-500">{listings.filter(l => l.type === 'seeking').length}</div>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end justify-center">
      <div className="bg-white w-full max-w-md rounded-t-3xl p-6 max-h-screen overflow-y-auto">
          ...
          <div className="flex gap-2">
            {['offering', 'seeking'].map(t => (
              <button key={t} onClick={() => setForm({...form, type: t})}
                className={`flex-1 py-3 rounded-xl text-sm font-bold capitalize transition-all ${form.type === t ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-500'}`}>
          ...
          <div className="grid grid-cols-2 gap-2">
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setForm({...form, category: cat})}
                className={`py-2 px-3 rounded-xl text-xs font-semibold transition-all text-left ${form.category === cat ? 'bg-teal-500 text-white' : 'bg-gray-50 text-gray-500'}`}>
                {cat}
              </button>
            ))}
          </div>
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-2">
            <input name="budgetMin" value={form.budgetMin} onChange={handle} placeholder="Min" type="number"
              className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-teal-400"/>
            <input name="budgetMax" value={form.budgetMax} onChange={handle} placeholder="Max" type="number"
              className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-teal-400"/>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-5">
      <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-sm border border-gray-100">
        ...
        <div className="mb-4">
          ...
          <input name="name" value={form.name} onChange={handle} placeholder="e.g. Rahul Yadav"
        ...
        <div className="mb-4">
          ...
          <input name="phone" value={form.phone} onChange={handle} placeholder="10 digit mobile number" type="tel" maxLength={10}
            className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-teal-400"/>
        </div>
        <div className="mb-4">
          <label className="text-xs font-bold uppercase tracking-wider text-gray-400 block mb-2">Your area</label>
        ...
        <div className="mb-4">
          ...
          <input name="city" value={form.city} onChange={handle} placeholder="e.g. Hyderabad"
            className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-teal-400"/>
        </div>
        <div className="mb-6">
        ...
        <div className="text-center mt-4 text-xs text-gray-400">By registering you agree to Worbid's terms of service</div>
      </div>
    </div>
  )
}
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-md mx-auto">
          ...
          <div className="flex gap-2 px-5 pb-4 overflow-x-auto">
            {cats.map(cat => (
              <button key={cat} onClick={() => setActivecat(cat)}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${activecat === cat ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
                {cat}
              </button>
            ))}
          </div>
        ...
        <div className="px-5 pt-4">
          ...
          {loading && <div className="text-center py-16 text-gray-400"><div className="text-3xl mb-3">⟳</div><div className="text-sm">Loading listings...</div></div>}
      ...
      {showModal && <PostModal onClose={() => setShowModal(false)} onSuccess={handlePostSuccess} currentUser={currentUser} />}
    </div>
  )
}
